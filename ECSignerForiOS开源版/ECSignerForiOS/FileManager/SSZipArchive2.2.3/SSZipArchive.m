...
+ (BOOL)isIpa:(NSString *)zipPath
{
    ...
    if ([fileName isEqualToString:@"Payload/"]) {
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    return [self createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory compressionLevel:Z_DEFAULT_COMPRESSION password:password AES:NO progressHandler:progressHandler];
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    if (success) {
        ...
        for (__strong NSString *fileName in allObjects) {
            ...
            if (!isDir && ![fullFilePath hasSuffix:@".app"]) {
                // file
