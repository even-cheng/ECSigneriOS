...
#define alertBottomMargin 0
//(([UIScreen mainScreen].bounds.size.height==812)||([UIScreen mainScreen].bounds.size.width==812) ? 34 : 0) // 适配iPhoneX

static NSString * const FOOTERCELL = @"footerCell";

#pragma mark ---------------------------- ECAlertAction begin --------------------------------

@interface ECAlertAction()

@property (nullable, nonatomic) NSString *title;
@property (nonatomic, assign) ECAlertActionStyle style;
@property (nonatomic, copy) void (^handler)(ECAlertAction *action);
// 当在addAction之后设置action属性时,会回调这个block,设置相应控件的字体、颜色等
// 如果没有这个block，那使用时，只有在addAction之前设置action的属性才有效
@property (nonatomic, copy) void (^propertyEvent)(ECAlertAction *action);
@end

@implementation ECAlertAction

- (id)copyWithZone:(NSZone *)zone {
    ECAlertAction *action = [[[self class] alloc] init];
...
+ (instancetype)actionWithTitle:(nullable NSString *)title style:(ECAlertActionStyle)style handler:(void (^ __nullable)(ECAlertAction *action))handler {
    ECAlertAction *action = [[self alloc] initWithTitle:title style:(ECAlertActionStyle)style handler:handler];
    return action;
    
}

- (instancetype)initWithTitle:(nullable NSString *)title style:(ECAlertActionStyle)style handler:(void (^ __nullable)(ECAlertAction *action))handler {
    if (self = [super init]) {
        ...
        if (style == ECAlertActionStyleDestructive) {
            self.titleColor = [UIColor redColor];
        } else if (style == ECAlertActionStyleCancel) {
...
#pragma mark ---------------------------- ECAlertAction end --------------------------------

#pragma mark ---------------------------- ECAlertControllerActionCell begin --------------------------------

@interface ECAlertControllerActionCell : UITableViewCell
@property (nonatomic, strong) ECAlertAction *action;
...
@implementation ECAlertControllerActionCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        // footerCell指的是ECAlertControllerStyleActionSheet下的取消cell和ECAlertControllerStyleAlert下actions小于_maxNumberOfActionHorizontalArrangementForAlert时的cell
...
- (void)setAction:(ECAlertAction *)action {
    _action = action;
...
@end
#pragma mark ---------------------------- ECAlertControllerActionCell end --------------------------------


#pragma mark ---------------------------- ECAlertController begin --------------------------------

@interface ECAlertController () <UIViewControllerTransitioningDelegate,UITableViewDataSource,UITableViewDelegate>
...
// action数组
@property (nonatomic) NSArray<ECAlertAction *> *actions;
// 取消样式的action数组
...
@property (nonatomic, assign) ECAlertControllerStyle preferredStyle;
@property (nonatomic, assign) ECAlertAnimationType animationType;
// 底部的cell数组
...
@property (nonatomic, assign) ECBackgroundViewAppearanceStyle backgroundViewAppearanceStyle;
...
+ (instancetype)alertControllerWithTitle:(nullable NSString *)title message:(nullable NSString *)message preferredStyle:(ECAlertControllerStyle)preferredStyle animationType:(ECAlertAnimationType)animationType {
    ...
    ECAlertController *alertController = [self alertControllerWithTitle:title message:message preferredStyle:preferredStyle animationType:animationType customView:nil];
    
    return alertController;
}

+ (instancetype)alertControllerWithTitle:(nullable NSString *)title message:(nullable NSString *)message preferredStyle:(ECAlertControllerStyle)preferredStyle animationType:(ECAlertAnimationType)animationType customView:(UIView *)customView {
    // 创建控制器
    ECAlertController *alertController = [[ECAlertController alloc] initWithTitle:title message:message preferredStyle:preferredStyle animationType:animationType customView:customView customHeaderView:nil customCenterView:nil customFooterView:nil];
    return alertController;
}
...
+ (instancetype)alertControllerWithPreferredStyle:(ECAlertControllerStyle)preferredStyle animationType:(ECAlertAnimationType)animationType customHeaderView:(nullable UIView *)customHeaderView {
    // 创建控制器
    ECAlertController *alertController = [[ECAlertController alloc] initWithTitle:nil message:nil preferredStyle:preferredStyle animationType:animationType customView:nil customHeaderView:customHeaderView customCenterView:nil customFooterView:nil];
    return alertController;
}

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(ECAlertControllerStyle)preferredStyle animationType:(ECAlertAnimationType)animationType customCenterView:(UIView *)customCenterView {
    // 创建控制器
    ECAlertController *alertController = [[ECAlertController alloc] initWithTitle:title message:message preferredStyle:preferredStyle animationType:animationType customView:nil customHeaderView:nil customCenterView:customCenterView customFooterView:nil];
    return alertController;
}

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(ECAlertControllerStyle)preferredStyle animationType:(ECAlertAnimationType)animationType customFooterView:(UIView *)customFooterView {
    // 创建控制器
    ECAlertController *alertController = [[ECAlertController alloc] initWithTitle:title message:message preferredStyle:preferredStyle animationType:animationType customView:nil customHeaderView:nil customCenterView:nil customFooterView:customFooterView];
    return alertController;
}
...
- (void)addAction:(ECAlertAction *)action {
    ...
    if (self.cancelActions.count && self.preferredStyle == ECAlertControllerStyleActionSheet) {
        if (action.style == ECAlertActionStyleCancel) { // 取消样式的按钮顺序排列
    ...
    if (self.preferredStyle == ECAlertControllerStyleActionSheet) {
        if (self.customCenterView && action.style != ECAlertActionStyleCancel) {
            NSLog(@"当自定义centerView时，ECAlertControllerStyleActionSheet下，除了取消样式的按钮之外，其余样式的按钮均不显示");
            [array removeObject:action];
            self.actions = array;
            [self layoutViewConstraints];
            return;
        }
        if (action.style == ECAlertActionStyleCancel && !self.customFooterView) {
        ...
        if (self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert && !self.customFooterView) {
            ...
            if (self.customCenterView) {
                NSLog(@"当自定义centerView时，ECAlertControllerStyleAlert下，action的个数最多只能是_maxNumberOfActionHorizontalArrangementForAlert个，超过_maxNumberOfActionHorizontalArrangementForAlert个的action将不显示");
                [array removeObject:action];
                self.actions = array;
                return;
            }
    ...
    action.propertyEvent = ^(ECAlertAction *action) {
        if (weakSelf.preferredStyle == ECAlertControllerStyleActionSheet) {
            if (action.style == ECAlertActionStyleCancel) {
                NSInteger index = [weakSelf.cancelActions indexOfObject:action];
                // 注意这个cell是与tableView没有任何瓜葛的
                ECAlertControllerActionCell *footerCell = [weakSelf.footerCells objectAtIndex:index];
            ...
            if (weakSelf.actions.count <= maxNumberOfActionHorizontalArrangementForAlert) {
                ...
                ECAlertControllerActionCell *footerCell = weakSelf.footerCells[index];
                footerCell.action = action;
            } else {
                [weakSelf.actionTableView reloadData];
            }
...
- (void)addTextFieldWithConfigurationHandler:(void (^)(UITextField * _Nonnull))configurationHandler {
    ...
    NSAssert(self.preferredStyle == ECAlertControllerStyleAlert, @"ECAlertControllerStyleActionSheet样式拒绝添加文本输入框");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ECAlertControllerActionCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ECAlertControllerActionCell class]) forIndexPath:indexPath];
    ECAlertAction *action = self.dataSource[indexPath.row];
    cell.action = action;
    return cell;
}
...
- (void)tableView:(UITableView *)tableView willDiEClayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    // 设置cell分割线整宽
...
#pragma mark - Private
- (instancetype)initWithTitle:(nullable NSString *)title message:(nullable NSString *)message preferredStyle:(ECAlertControllerStyle)preferredStyle animationType:(ECAlertAnimationType)animationType customView:(UIView *)customView customHeaderView:(UIView *)customHeaderView customCenterView:(UIView *)customCenterView customFooterView:(UIView *)customFooterView{
    ...
    if (self = [super init]) {
        ...
        if (animationType == ECAlertAnimationTypeDefault) {
            if (self.preferredStyle == ECAlertControllerStyleAlert) {
                animationType= ECAlertAnimationTypeAlpha;
            } else if (self.preferredStyle == ECAlertControllerStyleActionSheet) {
                animationType = ECAlertAnimationTypeRaiseUp;
            } else {
                animationType = ECAlertAnimationTypeRaiseUp;
            }
        }
...
- (void)setupViewsAboutAction:(UIView *)customCenterView {
    ...
    if (!customCenterView) {
        ...
        [actionTableView registerClass:[ECAlertControllerActionCell class] forCellReuseIdentifier:NSStringFromClass([ECAlertControllerActionCell class])];
...
- (void)createFooterCellWithAction:(ECAlertAction *)action {
    ...
    ECAlertControllerActionCell *footerCell = [[ECAlertControllerActionCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:FOOTERCELL];
...
- (void)clickedFooterCell:(UIButton *)sender {
    ...
    if (self.preferredStyle == ECAlertControllerStyleActionSheet) {
        ECAlertControllerActionCell *footerCell = (ECAlertControllerActionCell *)sender.superview;
        NSInteger index = [self.footerCells indexOfObject:footerCell];
        ECAlertAction *action = [self.cancelActions objectAtIndex:index];
        // 回调action的block
        if (action.handler) {
            action.handler(action);
        }
        ...
        if (self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert) {
            ECAlertControllerActionCell *footerCell = (ECAlertControllerActionCell *)sender.superview;
            NSInteger index = [self.footerCells indexOfObject:footerCell];
            ECAlertAction *action = [self.actions objectAtIndex:index];
            // 回调action的block
            if (action.handler) {
                action.handler(action);
            }
...
- (void)layoutHeader {
    ...
    if (!self.customHeaderView) { // 非自定义的headerView
        ...
        [labels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            // 子控件之间的垂直间距
            if (idx > 0) {
                NSLayoutConstraint *paddingConstraint = [NSLayoutConstraint constraintWithItem:label attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:labels[idx - 1] attribute:NSLayoutAttributeBottom multiplier:1.f constant:margin];
...
- (void)layoutFooter {
    ...
    if ((self.preferredStyle == ECAlertControllerStyleActionSheet && self.cancelActions.count) || (self.preferredStyle == ECAlertControllerStyleAlert && (self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert) && self.actions.count)) { // 需要footerBezelView
        if (self.preferredStyle == ECAlertControllerStyleAlert) { // alert样式
    ...
    if (!self.customFooterView) { // 不是自定义的footerView
        ...
        if (footerCells.count && ((self.preferredStyle == ECAlertControllerStyleActionSheet && self.cancelActions.count) || (self.preferredStyle == ECAlertControllerStyleAlert && self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert))) {
            [footerCells enumerateObjectsUsingBlock:^(ECAlertControllerActionCell *footerCell, NSUInteger idx, BOOL * _Nonnull stop) {
                if (self.preferredStyle == ECAlertControllerStyleAlert) {
                    // 设置footerCell的上下间距
...
- (CGFloat)actionBezelHeight:(CGFloat)footerTopMargin {
    ...
    if (self.actions.count) {
        ...
        } else if (self.preferredStyle == ECAlertControllerStyleAlert) {
            if (self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert) {
...
- (void)setMessageAlignment:(UITextAlignment)messageAlignment{
    ...
    self.detailTitleLabel.textAlignment = messageAlignment;
}
...
- (void)setOffsetYForAlert:(CGFloat)offsetYForAlert {
    ...
    ECAlertPresentationController *presentationVc = (ECAlertPresentationController *)self.presentationController;
...
- (void)keyboardWillChange:(NSNotification *)notification {
    ...
    CGRect alertRect = [self.alertView convertRect:self.alertView.frame toView:self.presentationController.containerView];
    CGFloat diff = fabs((ScreenHeight-keyboardEndY)*0.5);
    if (CGRectGetMaxY(alertRect) > keyboardEndY || diff == 0) {
...
- (nullable id <UIViewControllerAnimatedTransitioning>)animationControllerForPresentedController:(UIViewController *)presented presentingController:(UIViewController *)presenting sourceController:(UIViewController *)source {
    return [ECAlertAnimation animationIECresenting:YES];
}

- (nullable id <UIViewControllerAnimatedTransitioning>)animationControllerForDismissedController:(UIViewController *)dismissed {
    [self.view endEditing:YES];
    return [ECAlertAnimation animationIECresenting:NO];
}

- (nullable UIPresentationController *)presentationControllerForPresentedViewController:(UIViewController *)presented presentingViewController:(nullable UIViewController *)presenting sourceViewController:(UIViewController *)source NS_AVAILABLE_IOS(8_0) {
    return [[ECAlertPresentationController alloc] initWithPresentedViewController:presented presentingViewController:presenting];
}

@end

#pragma mark ---------------------------- ECAlertController end --------------------------------

@interface ECOverlayView: UIView
@property (nonatomic, strong) UIView *presentedView;
@property (nonatomic, assign) ECAlertController *alertController;
@property (nonatomic, strong) UIVisualEffectView *effectView;
@property (nonatomic, assign) CGFloat cornerRadiusForAlert;
- (void)redrawWithView:(UIView *)view alertController:(ECAlertController *)alertController;
@end
...
- (void)setAppearanceStyle:(ECBackgroundViewAppearanceStyle)appearanceStyle alpha:(CGFloat)alpha {
    switch (appearanceStyle) {
        case ECBackgroundViewAppearanceStyleTranslucent: {
            ...
            break;
        case ECBackgroundViewAppearanceStyleBlurExtraLight: {
            UIBlurEffect *blur = [UIBlurEffect effectWithStyle:UIBlurEffectStyleExtraLight];
            [self createVisualEffectViewWithBlur:blur alpha:alpha];
        }
            break;
        case ECBackgroundViewAppearanceStyleBlurLight: {
            UIBlurEffect *blur = [UIBlurEffect effectWithStyle:UIBlurEffectStyleLight];
            [self createVisualEffectViewWithBlur:blur alpha:alpha];
        }
            break;
        case ECBackgroundViewAppearanceStyleBlurDark: {
...
- (void)drawRect:(CGRect)rect {
    [super drawRect:rect];
    // 黑色带透明时进行镂空
    if (self.alertController.backgroundViewAppearanceStyle == ECBackgroundViewAppearanceStyleTranslucent && self.alertController.needDialogBlur && !self.alertController.textFields.count) {
        _cornerRadiusForAlert = (self.alertController.preferredStyle==ECAlertControllerStyleActionSheet)? 0 : self.alertController.cornerRadiusForAlert;
...
#pragma mark ---------------------------- ECAlertPresentationController begin --------------------------------

@interface ECAlertPresentationController()
@property (nonatomic, weak) ECOverlayView *overlayView;
@property (nonatomic, strong) NSMutableArray *presentedViewConstraints;
@end

@implementation ECAlertPresentationController
...
- (void)didChangeStatusBarOrientation:(NSNotification *)notification {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ECAlertController *alertController = (ECAlertController *)self.presentedViewController;
//        CGFloat cornerRadius = 0;
//        if (alertController.preferredStyle == ECAlertControllerStyleAlert) {
...
- (void)presentationTransitionWillBegin {
    ...
    ECOverlayView *overlayView = [[ECOverlayView alloc] init];
...
- (void)updateConstraints {
    ECAlertController *alertController = (ECAlertController *)self.presentedViewController;
    if (![alertController isKindOfClass:[ECAlertController class]]) {
        return;
    }
    ...
    if (!customView) { // 非自定义
        if (alertController.preferredStyle == ECAlertControllerStyleActionSheet) {
            ...
            if (alertController.animationType == ECAlertAnimationTypeDropDown || alertController.animationType == ECAlertAnimationTypeDropDownWithOutDown) {
        ...
        } else if (alertController.preferredStyle == ECAlertControllerStyleAlert) {
    ...
    @try{
        ...
        self.presentedViewConstraints = [NSMutableArray arrayWithArray:self.containerView.constraints];
    }
}
...
#pragma mark ---------------------------- ECAlertPresentationController end --------------------------------


#pragma mark ---------------------------- ECAlertAnimation begin --------------------------------

@interface ECAlertAnimation()
@property (nonatomic, assign) BOOL presenting;
@property (nonatomic, assign) CGFloat delayTime;
@end

@implementation ECAlertAnimation

- (instancetype)initWithPresenting:(BOOL)iECresenting {
    if (self = [super init]) {
        self.presenting = iECresenting;
        _delayTime = 0.22;
    }
    return self;
}

+ (instancetype)animationIECresenting:(BOOL)iECresenting {
    return [[self alloc] initWithPresenting:iECresenting];
}
...
- (void)presentAnimationTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    ECAlertController *alertController = (ECAlertController *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    CGSize controlViewSize = alertController.view.bounds.size;
    
    // 获取presentationController，注意不是presentedController
    ECAlertPresentationController *presentedController = (ECAlertPresentationController *)alertController.presentationController;
    ECOverlayView *overlayView = presentedController.overlayView;

    switch (alertController.animationType) {
        case ECAlertAnimationTypeRaiseUp:
            [self raiseUpWhenPresentForController:alertController
                                       transition:transitionContext
                                  controlViewSize:controlViewSize
                                      overlayView:overlayView];
            break;
        case ECAlertAnimationTypeDropDown:
        case ECAlertAnimationTypeDropDownWithOutDown:
...
- (void)dismissAnimationTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    ECAlertController *alertController = (ECAlertController *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    CGSize controlViewSize = alertController.view.bounds.size;
    // 获取presentationController，注意不是presentedController
    ECAlertPresentationController *presentedController = (ECAlertPresentationController *)alertController.presentationController;
    ECOverlayView *overlayView = [ECOverlayView new];
    if ([presentedController isKindOfClass:[ECAlertPresentationController class]]) {
        overlayView = presentedController.overlayView;
    }
    
    switch (alertController.animationType) {
        case ECAlertAnimationTypeRaiseUp:
        case ECAlertAnimationTypeDropDownWithOutDown:
            [self dismissCorreECondingRaiseUpForController:alertController
                                                transition:transitionContext
                                           controlViewSize:controlViewSize
                                               overlayView:overlayView];
            break;
        case ECAlertAnimationTypeDropDown:
            [self dismissCorreECondingDropDownForController:alertController
                                                 transition:transitionContext
                                            controlViewSize:controlViewSize
                                                overlayView:overlayView];
            break;
        ...
        case ECAlertAnimationTypeAlpha:
            [self dismissCorreECondingAlphaForController:alertController
                                              transition:transitionContext
                                         controlViewSize:controlViewSize
                                             overlayView:overlayView];
            break;
        case ECAlertAnimationTypeExpand:
            [self dismissCorreECondingExpandForController:alertController
                                               transition:transitionContext
                                          controlViewSize:controlViewSize
                                              overlayView:overlayView];
            break;
        case ECAlertAnimationTypeShrink:
            [self dismissCorreECondingShrinkForController:alertController
                                               transition:transitionContext
                                          controlViewSize:controlViewSize
                                              overlayView:overlayView];
            break;
...
// 从底部忘上弹的present动画
- (void)raiseUpWhenPresentForController:(ECAlertController *)alertController
                             transition:(id<UIViewControllerContextTransitioning>)transitionContext
                        controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
...
// 从底部往上弹对应的dismiss动画
- (void)dismissCorreECondingRaiseUpForController:(ECAlertController *)alertController
                             transition:(id<UIViewControllerContextTransitioning>)transitionContext
                        controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
...
// 从顶部往下弹的present动画
- (void)dropDownWhenPresentForController:(ECAlertController *)alertController
                              transition:(id<UIViewControllerContextTransitioning>)transitionContext
                         controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
...
// 从顶部往下弹对应的dismiss动画
- (void)dismissCorreECondingDropDownForController:(ECAlertController *)alertController
                                      transition:(id<UIViewControllerContextTransitioning>)transitionContext
                                 controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
    [overlayView redrawWithView:nil alertController:nil];
...
- (void)alphaWhenPresentForController:(ECAlertController *)alertController
                             transition:(id<UIViewControllerContextTransitioning>)transitionContext
                        controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
...
- (void)dismissCorreECondingAlphaForController:(ECAlertController *)alertController
                                       transition:(id<UIViewControllerContextTransitioning>)transitionContext
                                  controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
    [overlayView redrawWithView:nil alertController:nil];
...
// 发散的prensent动画
- (void)expandWhenPresentForController:(ECAlertController *)alertController
                           transition:(id<UIViewControllerContextTransitioning>)transitionContext
                      controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
...
// 发散对应的dismiss动画
- (void)dismissCorreECondingExpandForController:(ECAlertController *)alertController
                                    transition:(id<UIViewControllerContextTransitioning>)transitionContext
                               controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
    [overlayView redrawWithView:nil alertController:nil];
...
// 收缩的present动画
- (void)shrinkWhenPresentForController:(ECAlertController *)alertController
                            transition:(id<UIViewControllerContextTransitioning>)transitionContext
                       controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
...
// 收缩对应的的dismiss动画
- (void)dismissCorreECondingShrinkForController:(ECAlertController *)alertController
                                     transition:(id<UIViewControllerContextTransitioning>)transitionContext
                                controlViewSize:(CGSize)controlViewSize overlayView:(ECOverlayView *)overlayView {
    // 与发散对应的dismiss动画相同
    [self dismissCorreECondingExpandForController:alertController transition:transitionContext controlViewSize:controlViewSize overlayView:overlayView ];
}

@end

#pragma mark ---------------------------- ECAlertAnimation end --------------------------------
